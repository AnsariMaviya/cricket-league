...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-blue-600 text-white px-6 py-4">
            <h1 class="text-xl font-semibold">Edit Country: {{ $country->name }}</h1>
        </div>
        <form action="{{ route('countries.update', $country) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-6">
                <label for="short_name" class="block text-sm font-medium text-gray-700 mb-2">Short Name</label>
                <input type="text" name="short_name" id="short_name" value="{{ old('short_name', $country->short_name) }}" required maxlength="10"
