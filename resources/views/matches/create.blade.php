...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-red-600 text-white px-6 py-4">
            <h1 class="text-xl font-semibold">Add New Match</h1>
        </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <select name="first_team_id" id="first_team_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        ...
                        <option value="{{ $team->team_id }}" {{ old('first_team_id') == $team->team_id ? 'selected' : '' }}>
                            {{ $team->team_name }}
                        </option>
                        @endforeach
                    </select>
                    @error('first_team_id')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-4">
                    ...
                    <select name="second_team_id" id="second_team_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        <option value="">Select Team</option>
                        @foreach($teams as $team)
                        <option value="{{ $team->team_id }}" {{ old('second_team_id') == $team->team_id ? 'selected' : '' }}>
                            {{ $team->team_name }}
                        </option>
                        @endforeach
                    </select>
                    @error('second_team_id')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="match_date" id="match_date" value="{{ old('match_date') }}"
                ...
                <div class="mb-4">
                    ...
                    <select name="match_type" id="match_type" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        ...
                        <option value="ODI" {{ old('match_type') == 'ODI' ? 'selected' : '' }}>ODI</option>
                        <option value="Test" {{ old('match_type') == 'Test' ? 'selected' : '' }}>Test</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label for="overs" class="block text-sm font-medium text-gray-700 mb-2">Overs</label>
                    <input type="number" name="overs" id="overs" value="{{ old('overs', 20) }}" required min="1"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="first_team_score" id="first_team_score" value="{{ old('first_team_score') }}" placeholder="e.g., 180/5"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                </div>
                <div class="mb-4">
                    <label for="second_team_score" class="block text-sm font-medium text-gray-700 mb-2">Team 2 Score</label>
                    <input type="text" name="second_team_score" id="second_team_score" value="{{ old('second_team_score') }}" placeholder="e.g., 175/8"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                </div>
            </div>
            <div class="mb-6">
                <label for="outcome" class="block text-sm font-medium text-gray-700 mb-2">Outcome</label>
                <input type="text" name="outcome" id="outcome" value="{{ old('outcome') }}" placeholder="e.g., Team A won by 5 runs"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>
